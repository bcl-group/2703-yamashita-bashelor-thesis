...
\begin{align}
  ...
  \beta_h(V)  &= \frac{1}{\exp(3 - 0.1V) + 1} \label{eq:rate}\\
  \alpha_n(V) &= \frac{0.1 - 0.001V}{\exp(1 - 0.1V) - 1}, &
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrr}
    ...
    \midrule
    $\mathrm{Na}^{+}$      &  115.0 & 120.0 \\
    $\mathrm{K}^{+}$       &  -12.0 &  36.0 \\
    その他のイオン (leak)   &   10.6 &   0.3 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\TODO{本研究の HH 実装では $V$ の基準をどう取っているか（$-65$ mV か $0$ mV か）を
      確認して統一する。式\eqref{eq:rate}のレート関数は $V$ の基準に依存する}
